...
try {
	...
	$reponse = $bdd->query ( "SELECT id, uuid, nom, createur, contributeurs, difficulte, qualite, etat, idServOrigine, versionMin, verConversion FROM maps WHERE etat = 'disponible' OR etat = 'supprimee' ORDER BY points DESC" ) or erreurAPI ( "SQL error 25" );
	...
	// Recherche des tricheurs avérés non vérifiés
	// Récupération de la liste des opérateurs et des tricheurs ignorés du serveur pour ne pas les considérer comme tricheurs
	$reponse = $bdd->prepare ( "SELECT idsProprietaires, tricheursIgnores, operateurs FROM serveurs WHERE id = :id AND operateurs <> ''" );
	$reponse->execute ( array (
			'id' => $idServ 
	) ) or die ( "SQL error 169" );
	$d = $reponse->fetch ();
	$reponse->closeCursor ();
	if ($d) {
		$idsProprietaires = array_filter ( explode ( ";", $d ["idsProprietaires"] ) );
		$tricheursIgnores = array_filter ( array_merge ( explode ( ";", $d ["tricheursIgnores"] ), explode ( ";", $d ["operateurs"] ) ) );
		
		// Liste des tricheurs et comptage
		$reponse = $bdd->prepare ( "SELECT uuidJoueur FROM fantomes WHERE etat = 'invalide' AND idServ = :id AND triche = 1 AND cache = 0" );
		$reponse->execute ( array (
				'id' => $idServ 
		) ) or die ( "SQL error 179" );
		$nbTricheurs = 0;
		while ( $d = $reponse->fetch () ) {
			if (! in_array ( $d ["uuidJoueur"], $tricheursIgnores ))
				$nbTricheurs ++;
		}
		$reponse->closeCursor ();
		
		if ($nbTricheurs) {
			ajouterReponse ( "nbTricheurs", $nbTricheurs );
			
			// Récupération de la liste des joueurs pouvant être notifiés
			$ids = str_repeat ( '?,', count ( $idsProprietaires ) - 1 ) . '?';
			$reponse = $bdd->prepare ( "SELECT minecraftUUID FROM utilisateurs WHERE id IN (" . $ids . ") AND notifsTricheursJeu = TRUE" );
			$reponse->execute ( $idsProprietaires ) or die ( "SQL error 193" );
			while ( $d = $reponse->fetch () ) {
				$receveurs [] = $d ["minecraftUUID"];
			}
			$reponse->closeCursor ();
			ajouterReponse ( "receveursNotifsTriche", $receveurs );
		}
	}
} catch ( Exception $e ) {
