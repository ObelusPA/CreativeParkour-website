...
try {
	...
	while ( $donnees = $reponse->fetch () ) {
		...
		if (array_key_exists ( $nomFantome, $fantomesServ )) { // Si le fantôme est sur le serveur
			...
			$telecharger = true;
		if ($telecharger && $_POST ["telechargementFantomesAutorise"] == "true" && $donnees ["etat"] == "valide" && ! in_array ( $nomFantome, $fantomesExclus )) { // Si le téléchargement est autorisé et que le fantôme n'est pas sur le serveur et que le serveur a la map
