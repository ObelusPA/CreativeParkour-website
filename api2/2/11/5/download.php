...
// Données POST envoyées : ipJoueur, uuidJoueur, idMap, versionServ
try {
	...
	if ($matches [1])
		$idMap = ( int ) $matches [1];
			...
			$reponse = $bdd->prepare ( "SELECT id, nom, uuid, createur, contributeurs, taille, fichierContenu, difficulte, qualite, detailsCompat, verConversion FROM maps WHERE id = :idMap AND etat = 'disponible'" );
			...
			if ($donnees) {
				$idMap = $donnees ["id"];
				$uuidCreateur = separerUuidNom ( $donnees ["createur"] ) ["uuid"];
				ajouterReponse ( "nomMap", $donnees ["nom"] );
				ajouterReponse ( "uuidMap", $donnees ["uuid"] );
				ajouterReponse ( "createur", $uuidCreateur . ":" . getNomMC ( separerUuidNom ( $donnees ["createur"] ) ["uuid"] ) );
				ajouterReponse ( "contributeurs", explode ( ";", $donnees ["contributeurs"] ) );
				ajouterReponse ( "difficulte", $donnees ["difficulte"] );
				ajouterReponse ( "qualite", $donnees ["qualite"] );
				ajouterReponse ( "verConversion", $donnees ["verConversion"] );
				ajouterReponse ( "detailsCompat", $donnees ["detailsCompat"] );
				if ($donnees ["verConversion"] <= $_POST ["versionServ"]) {
