...
// Renvoie les données détaillées d'un fantôme au serveur (avec les checkpoints et les positions, contrairement à list.php), avec le profil du joueur
...
try {
	...
	foreach ( $_POST as $post => $dPost ) {
		if (strpos ( $post, "fantome" ) !== false) { // Si c'est bien un fantôme
			...
			$reponse = $bdd->prepare ( "SELECT uuidMap, uuidJoueur, date, ticks, millisecondes, fichierFantome FROM fantomes WHERE uuidMap = :uuidMap AND uuidJoueur = :uuidJoueur AND etat = :etat ORDER BY date DESC LIMIT 1" );
			...
			if ($donnees) {
				// Lecture du fichier
				$zp = gzopen ( "/home/creativeeb/www/maps data/ghosts/" . $donnees ["fichierFantome"] . ".gz", 'r' );
				if (! is_bool ( $zp )) {
					while ( ! gzeof ( $zp ) ) {
						$buffer .= gzread ( $zp, 4096 );
					}
					gzclose ( $zp );
					$profil = getProfilMC ( $donnees ["uuidJoueur"] );
					$fantomesATelecharger [] = array (
							...
							"texturesProfil" => $profil ["textures"],
							"signatureProfil" => $profil ["signature"]
					);
					unset ( $buffer );
				}
			}
